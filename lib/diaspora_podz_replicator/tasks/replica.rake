...
self.logdir = "#{Dir.pwd}/log"

def pod_count
  DiasporaPodzReplicator.pod_count
end
...
namespace :replica do
  ...
  def launch_pod(pod_nr)
    if diaspora_up?(DiasporaPodzReplicator.pod_uri(pod_nr))
      logger.info "Pod number #{pod_nr} is already up!"
    else
      eye("start", "test", "SERVER_URL='#{DiasporaPodzReplicator.pod_host(pod_nr)}'")
    end
    eye("info", "test", "SERVER_URL='#{DiasporaPodzReplicator.pod_host(pod_nr)}'", true)
  end

  def stop_pod(pod_nr)
    if diaspora_up?(DiasporaPodzReplicator.pod_uri(pod_nr))
      eye("stop", "test", "SERVER_URL='#{DiasporaPodzReplicator.pod_host(pod_nr)}'")
    else
  ...
  def deploy_app_revision(pod_nr, revision)
    ...
    Bundler.with_clean_env do
      deploy_app("test", "BRANCH=#{revision} SERVER_URL='#{DiasporaPodzReplicator.pod_host(pod_nr)}'")
    end
  end
  ...
  task :check_repository_clone do
    within_diaspora_replica do
      pipesh "ln -fs -T #{DiasporaPodzReplicator.diaspora_root}/ src"
      `cd src && git status`
      # if $? == 0
  ...
  task deploy_apps: :bring_up_testfarm do
    (1..pod_count).each do |i|
      deploy_app_revision(i, DiasporaPodzReplicator.configuration["pod#{i}"]["revisions"].first)
    end
  end
  ...
  task launch_pods: :bring_up_testfarm do
    if testenv_off?
      ...
      (1..pod_count).each do |i|
        next if wait_pod_up(DiasporaPodzReplicator.pod_uri(i))
  ...
  def deploy_and_launch_testfarm(pod_nr=1, &f)
    DiasporaPodzReplicator.configuration["pod#{pod_nr}"]["revisions"].each do |revision|
